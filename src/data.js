...
const CHANGELOG = [
  {
    v: '2026.09.14.1', date: '2026-09-14',
    title: '英文套装名跟随切换 + 手机开屏公告改小卡片 + 日志按「日期·子版本」归并',
    items: [
      '切换英文（数据语言 English）时圣遗物套装名跟随切换为官方英文名：覆盖角色卡配装芯片、配装编辑抽屉标题、副词条表套装列、以及「复制配装」下拉选项（此前这些位置仍残留中文名）。',
      '手机端（≤560px）开屏更新公告由全屏改为小巧居中的可滚动卡片：日志区域独立上下滑动，底部「知道了」按钮常驻可见，短公告不再被撑成满屏高框。',
      '更新日志改为按「日期 + 同日子版本号」归并：同一天的多次更新用 2026.09.14.1 / 2026.09.14.2 区分，避免每天散出多条重复日期的条目。',
    ],
  },
  {
    v: '2026.09.14', date: '2026-09-14',
    title: '手机端适配修复：顶栏空间优化 + 弹窗全屏',
    items: [
      '手机端（≤560px）顶栏：语言按钮「中 / EN」进一步压缩内边距；帮助按钮「使用说明」改为「?」图标，释放横向空间给标题；标题字号从 15px 降到 14px，减少省略。',
      '手机端弹窗（更新公告 / 使用说明 / 角色编辑 / 配装编辑 / 属性选择 / 套装管理 / 规则表单）统一改为 100vw × 100vh 全屏，取消圆角与边框，底部加安全区留白；解决 94vw 卡片在 390px 屏上内容拥挤的问题。',
    ],
  },
  {
    v: '2026.09.13', date: '2026-09-13',
    title: '数据按观测枢 wiki 整体重核对 + 中英文切换定稿 + 配装功能定位',
    items: [
      '中英文切换定稿：顶栏【单个按钮】外观「中 / EN」两段（当前语言金色高亮），点一下同时切界面文案与角色 / 套装 / 属性名；顶栏顺序「保存 → 语言切换 → 使用说明」；数据管理「关于数据」5 条说明补英文并补回动态计数（applyI18n 整段替换会重置计数，已修复）。',
      '配装数据整体按米游社观测枢 wiki「推荐装备」结构化数据重核对：124 角色主词条按 wiki 原文顺序排序、2+2 混搭识别保留、四星过渡套不混入正式配装；攻略来源第 1 条固定观测枢词条（真数据源），后挂筛选过的米游社攻略并直接显示作者名。',
      '每组配装独立携带 套装 + 三部位主词条 + 副词条 + 流派标签（主C/副C/辅助，由 wiki 启发式推断、可手改），推翻旧 SUB_PRESETS 粗桶——心海等 12 角色网页双暴显示纠正为 wiki 真实值；同角色多流派分组保留不再坍缩。',
      '配装新增「功能定位」多选 + 自定义（输出 / 增伤 / 减抗 / 治疗 / 护盾 / 增幅反应 / 剧变反应…），跨角色共享自定义词表；全部 124 角色定位按 wiki 重新推断写入。',
      '角色与圣遗物按图鉴核对：角色库补全到 124（新增挪德卡莱国度、旅行者按 6 元素形态拆分、补洛恩等 6 角色），圣遗物 45→46 套并订正两个套装名；数据语言切 English 显官方英文名、搜索中英文都认。',
      '界面打磨：套装名与配装切换合并一行可点选、流派切换芯片去色点只留编号、详情「攻略来源」标注数据源 / 攻略、锁定方案按「重要属性」相似度自动合并 + 命中条数可调、手机端配装编辑横向滚动修复、② ③ 页加图鉴 / 推荐排序与正序 / 倒序切换。',
      '修「中文模式出现英文」的误解：实为本地存储残留了之前切到的 en 状态；单按钮直接显示当前语言，看到 EN 点一下即切回。',
    ],
  },
  {
