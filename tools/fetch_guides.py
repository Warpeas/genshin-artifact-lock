...
选取规则（优先级从高到低）：
  1. 作者白名单优先（见 `AUTHORS`：Asgater / HoYo青枫 / 幕陵 / 风伤幽冥 / 猫冬 / Sattle），每位各取一篇
...
# 选取逻辑对所有角色统一：白名单作者优先各取一篇，不够再按热度回退（见 pick()）。
# 早期只有 Asgater/HoYo青枫，导致水/火旅行者拉不到白名单稿、只能走热度回退（与冰不一致）；
# 故把覆盖水/火的作者也并入同一白名单，水/火与冰走完全相同的筛选路径。
# 幕陵/Sattle 主水，风伤幽冥/猫冬主火（猫冬亦覆盖岩/水，Sattle 亦覆盖风/雷/草/水）。
AUTHORS = {
    "79695828": "Asgater",
    "285802042": "HoYo青枫",
    "195479393": "幕陵",
