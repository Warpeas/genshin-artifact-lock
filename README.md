...
1. **角色分组**：提取每个角色的需求特征（沙/杯/冠主词条集合 + 副词条权重向量），把需求「几乎一致」（相似度 ≥ 0.95）的角色自动并为一组，其余保持独立——**组数不强行压到 3**
2. **归入方案槽**：
   - 组数 ≤ 3：一组一个方案，直接生成
...
4. **★必须**：组内**全员共有**的核心副词条（权重 ≥ 0.4），按平均权重降序，最多 2 个
5. **候选池**：组内任一角色需要的副词条，剔除与已选主词条冲突的项
