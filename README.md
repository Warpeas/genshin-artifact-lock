...
- 内置 113 个可配装角色的常见配装（覆盖至 7.0 至冬版本），每个角色带「国度 + 队伍定位」标签；配装整理自社区常见推荐思路，6.6–7.0 新角色的配装可能随环境微调，建议在角色配置页按自己的练度再核一遍。剧情 NPC（如尼可、洛恩、雅珂达、莉奈娅等）无社区配装共识，故未纳入；套装名均对照米游社/米哈游百科核验，不含编造条目
- 每个角色卡片底部标「🔗 攻略来源」，链接来自**米游社观测枢角色攻略频道**（[https://baike.mihoyo.com/ys/strategy/channel/map/37/39](https://baike.mihoyo.com/ys/strategy/channel/map/37/39)），按「养成 / 配装 / 一图流 / 解析」关键词为每个角色挑选最相关的 1–3 篇攻略（如 `miyoushe.com/ys/article/xxxxxx`）。来源只展示真实攻略 URL、不做任何文字总结；找不到攻略的角色（如依鲁加）留空。编辑抽屉的「攻略来源链接」框**每行一个链接、可列多个**，仅用于存档你认可的配装攻略，**工具不作任何解析**；你也可以换成自己信得过的链接。所有内置配装均对照米游社核验、不含编造条目
