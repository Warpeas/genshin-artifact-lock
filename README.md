...
- **游戏内锁定方案**（核心产出）：按游戏内「套装锁定方案」的真实规则生成，可进游戏照搬
- 元素伤害杯自动打「稀有·建议多留」标记
- 支持复制清单 / 复制游戏内方案、导出 CSV、打印 / 存 PDF

#### 游戏内锁定方案

对齐游戏内「背包 → 圣遗物 → 锁定功能 → 套装锁定方案」的实际规则（依据 HoYoWiki 官方 wiki 与社区攻略）：

| 规则 | 说明 |
|---|---|
| 预设数量 | 每种套装**至多 2 个自定义方案**（官方上限），多个方案在锁定时共同生效 |
| 可设部位 | 时之沙 / 空之杯 / 理之冠可分别设主要属性与追加属性；生之花 / 死之羽主词条固定，只能设追加属性 |
| ★必须 | 勾选带 ★ 的追加属性，表示必须命中 |
| 包含任意 N 条 | 在候选词条中至少命中 N 条（★计入总数），N 取 1–4 |
| 3 词条圣遗物 | 仅有 3 条追加属性时，所需数量自动减 1 |

**关键：为什么是「聚类合并」而不是简单汇总**

攻略明确指出——**需求差异大的角色不宜并入同一方案，否则会「存伪」**（把别的角色用不上的圣遗物也锁进来）。由于槽位只有 2 个，本工具的做法是：

1. 提取每个角色的需求特征（三部位主词条集合 + 副词条权重向量）
2. 用**层次聚类**按相似度合并，相似度低于阈值（0.62）的组不会被强行并到一起
3. 同组角色的需求才合并成一个方案：主属性取并集（最多 3 个）、★必须取全员共有的核心词条（最多 2 个，通常是双暴）、命中数取组内平均核心词条数
