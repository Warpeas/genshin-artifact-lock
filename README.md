...
- 每个角色卡片底部标「🔗 攻略来源」，链接取自**米游社角色攻略频道**（[https://baike.mihoyo.com/ys/strategy/channel/map/37/39](https://baike.mihoyo.com/ys/strategy/channel/map/37/39)）等真实攻略来源（不限定平台，亦可用 Game8 / KQM / B站 等），按「养成 / 配装 / 一图流 / 解析」为每个角色挑选最相关的 1–3 篇攻略（如 `miyoushe.com/ys/article/xxxxxx`），均真实可访问、不重复；找不到合适攻略的角色留空数组。编辑抽屉的「攻略来源链接」改为**一行一条 + 加减按钮**，可任意增删你认可的配装攻略，**工具不作任何解析**；你也可以换成自己信得过的链接。所有内置配装均对照米游社核验、不含编造条目
