...
- **🧩 散件 / 过渡 保留规则**：页面顶部可开关与具体角色无关的保留条件（如稀有的元素伤害杯）。启用后每条规则**占用一个预设槽位**（每套上限 3 个），角色需求合并方案会自动压缩让位，总数始终不超限；规则生成的卡片以绿色左边框和「🧩 散件 / 过渡保留」标题区分。内置 4 条（元素伤害杯 / 双暴头 / 充能沙 / 精通杯，默认启用元素伤害杯），也支持自建规则（选部位、主词条、候选副词条、包含任意 N 条）
- **手动合并**：某套装的需求分组超过可用槽位时，出现「✂️ 手动合并」面板——每个分组一行（角色 + 主属性摘要 + 归入方案 1/2/3 的下拉），自动结果已预填，可自行调整；改动保存在本地存档，「重置为自动合并」一键还原
...
1. **角色分组**：提取每个角色的需求特征（沙/杯/冠主词条集合 + 副词条重要度向量），把需求「几乎一致」（相似度 ≥ 0.95）的角色自动并为一组，其余保持独立——**组数不强行压到 3**
2. **归入方案槽**：
   - 组数 ≤ 3：一组一个方案，直接生成
   - 组数 > 3：出现「✂️ 手动合并」面板，列出每个分组（角色 + 主 / 副属性需求摘要），**默认按自动结果预填**（贪心归并最相似的组），你可以把任意组改归到方案 1/2/3；改动随本地存档保存，「重置为自动合并」一键还原
3. **主属性**：花/羽固定；沙/杯/冠按「票数 × 重要度」降序取到累计覆盖 ≥ 70% 为止，上限 3 个
4. **★必须**：组内**全员都勾了「必选」**的副词条，按平均重要度降序，最多 2 个
