...
| 主词条优先级 | 第 1 条最优；其后每条 `=` 同权 / `>` 降权 ×0.72 | 1.0 / 1.0 / 0.72 … |
...
副词条不再用 0–1 数值权重，而是用**「排序 + 必选 + 重要度算子」**表达。界面上每条（首条除外）都有一个 `=` / `>` 开关，外加「上下移」和「☆/★」：

```js
// 一份副词条需求 = 有序数组；req = ★必须；op 表示与上一条的重要度关系
...
- 点卡片打开编辑抽屉，可改：配装组（4 件套 / 2+2）、**主词条（`=` / `>` 重要度 + 排序）**、**副词条（`=` / `>` 重要度 + ★必选）**、国度与定位；支持新增自定义角色
...
- **🧩 散件 / 过渡 保留规则**：页面顶部可开关与具体角色无关的保留条件（如稀有的元素伤害杯）。每条启用的规则会生成一份**候选方案**，与角色聚类结果**同权**参与「并入 / 采纳」，不再单独占用槽位；规则卡片以绿色左边框和「🧩 散件 / 过渡保留」标题区分。内置 4 条（元素伤害杯 / 双暴头 / 充能沙 / 精通杯，默认启用元素伤害杯），也支持自建规则（选部位、主词条、候选副词条、包含任意 N 条）
...
| 主副词条互斥 | 副词条不可能与同部位主词条相同。方案级统一副词条后，仅自动剔除与花 / 羽固定主词条（生命值 / 攻击力）冲突的项；沙 / 杯 / 冠的主词条随方案而变，无法逐部位剔除——照抄时请自行留意（例如方案主要属性选了充能，则副词条池里的充能效率对该部位是无效勾选，跳过即可） |

**固定算法（按角色的追加属性需求聚类，结果可复现）**

攻略明确指出——**需求差异大的角色不宜并入同一方案，否则会「存伪」**（把别的角色用不上的圣遗物也锁进来）。工具不再替你压到 3 套，而是给出**全部候选 + 手动整理**：

1. **角色分组**：以**副词条需求为主**（余弦相似度 0.75 权重）+ 主属性为辅，对使用该套装的角色做凝聚聚类。
   每一步都合并「当前最相似的两组」，但**是否停手由「方案贴合度」决定**——合并后的条件若稀释到贴合度下限（默认 0.72，滑块可调）以下，就保留为独立候选。贴合度是连续量（按名次加权的需求覆盖率 + 候选池宽度），滑块不会出现「一档跳到底」的台阶感
2. **候选合成**：每组角色生成一份候选方案——**全方案唯一一份追加属性条件**（★必须 = 组内全员必选的交集，最多 2 个；候选池 = 组内任一角色前 5 条需求，最多 5 条；包含任意 N 条按最宽松部位系数取值），主要属性逐部位按「票数 × 重要度」降序取到累计覆盖 ≥ 70%，上限 3 个
3. **散件 / 过渡规则同权加入**：每条启用的保留规则转成一份候选（主词条稀有条件 + 追加属性条件），与角色聚类候选一起参与后续整理
4. **手动整理**：用「并入…」把候选并成一套（★必须取交集、候选池取并集、包含条数取更宽松值），用「采纳」勾选决定最终要照抄的方案；超过每套 3 个时页面顶部提醒
